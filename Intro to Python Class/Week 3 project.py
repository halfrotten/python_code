...
for i in cleaning_service_info.split(","):
    print(i)
while True:   
    try:
...
while True:
    type_of_cleaning=input("type of cleaning? (standard:1 or full house:2)?")
    if type_of_cleaning.find("full")==0 or type_of_cleaning=="2":
            print(CostOfCleaning("2",number_of_rooms))
            break
    elif type_of_cleaning.find("stand")==0 or type_of_cleaning=="1":
            print(CostOfCleaning("1",number_of_rooms))
            break
    else:
